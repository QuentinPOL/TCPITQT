...
void ServeurTCPIP::onClientReadyRead()
{
    ...
    else if (str.left(2) == "Tf" && isInt && isInt2) // Si c'est une demande de température en °F
    {
        ...
        str = "Tf" + QString::number(int1) + QString::number(int2) + QString::number(randomTemp, 'f', 2); // On fait la réponse et on met une précision de 2 chiffres derrière la virgule pour un float
    }
